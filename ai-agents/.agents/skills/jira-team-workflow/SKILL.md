...
Основной поток: `NEW → BACKLOG → TO DO → DEVELOPING → REVIEW → RELEASE PREPARATION → DONE`. Есть обратные переходы и служебный `TRASHED` из любого статуса. Источник истины - плейбук `docs/playbooks/jira-workflow.md` в репозитории `dwsai-data-agent`.

## Статусы

| Статус | Значение | Куда дальше |
|--------|----------|-------------|
| `NEW` | Общий беклог, задача заведена и не разобрана | `BACKLOG` после груминга и приоритизации |
| `BACKLOG` | Приоритетный беклог, из него набирается текущая работа | `TO DO` при взятии в работу; `NEW` при возврате |
| `TO DO` | Разобрана, готова к работе, точка взятия ответственности | `DEVELOPING` при старте; `BACKLOG` при возврате постановки |
...
- `NEW → BACKLOG`: задача разобрана и приоритизирована.
- `BACKLOG → TO DO`: задача готова к работе, есть оценка, сняты блокеры.
- `TO DO → DEVELOPING`: взяли в работу, задачу ведет один человек.
- `DEVELOPING → REVIEW`: открыт MR, задача связана с ним. Доработки по ревью идут внутри `REVIEW`, задачу назад не двигаем.
- `REVIEW → RELEASE PREPARATION`: после approve и мержа.
- `RELEASE PREPARATION → DONE`: после релиза.
- Обратные (`DEVELOPING → TO DO`, `TO DO → BACKLOG`, `BACKLOG → NEW`): работу нельзя продолжать.
...
Полный цикл доставки задачи (Jira -> ветка -> MR -> зеленый pipeline -> Review) оркеструет skill `work-task-delivery`; этот skill остается источником истины для точечных Jira-операций.
