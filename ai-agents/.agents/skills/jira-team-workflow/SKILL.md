...
description: >-
  ...
  Используй этот skill всякий раз, когда пользователь хочет ДВИГАТЬ, СМОТРЕТЬ или ОПУБЛИКОВАТЬ задачи:
  ...
  «workflow проекта DWSAI», «как устроен наш процесс в джире», «опубликуй готовый draft в трекер», «заведи задачу через dp_jira_create».
  Срабатывает, даже если слово «JIRA» не названо явно, но речь про статус, переход, ревью, релиз,
  публикацию готовой задачи или ключ задачи вида DWSAI-XXX.
  Декомпозиция и оформление новых задач из PRD, RFC, Epic живут в jira-issue-author (он готовит draft);
  сюда входит публикация этого готового draft в трекер через dp_jira_create. Этот skill про движение, инспекцию и публикацию задач.
---
