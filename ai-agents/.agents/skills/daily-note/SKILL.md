...
- **Never edit weekly-review files:** the weekly log `02_weekly_log.md`, anything under a weekly `_workspace/*`, `MATRIX`, `MAPPING`, quarterly notes, or templates. Those have their own structure; restructuring them here corrupts the weekly-review workflow.
- Do not do a broad multi-file pass. "Tidy all of this week's dailies" is not this skill — clean one note per invocation, or hand the week to weekly-review.
...
When structure is requested, use these headings (named to match weekly-review day fields so consolidation maps cleanly):
...
### How these map into weekly-review

- `Planned / Done / Blockers / Decisions / Jira / Artifacts / Learn / Research / Attention` → same-named weekly-review **day fields** (1:1).
- `In progress` → unchecked tasks; weekly-review carries these forward (Carry-forward), not as achievements.
- `Decisions` → weekly-review surfaces these in **1:1 talking points** / Staff signal — keep them, they are high-value for Staff evidence.
- `Questions` → weekly-review Back Matter.

## Pairing with weekly-review (ordering)

- daily-note is the **write side** for one daily note; weekly-review treats daily notes as **read-only** and consolidates them into the weekly log.
- Structure a daily note **before** `consolidate`. Once a day has been consolidated into `02_weekly_log.md`, do **not** re-edit that day's note here — it would desync the weekly log, break consolidate idempotency, and bump `updated` away from the real work day (weakening evidence). If a consolidated day must change, re-run weekly-review `consolidate` afterwards.
- Never edit a source daily note as part of a weekly-review flow — that is this skill's job, run separately and first.
