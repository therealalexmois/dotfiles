...
Вне scope этого skill: merge MR, approve, переходы `REVIEW -> RELEASE PREPARATION -> DONE` и релиз (`work-release-manager`), код-ревью чужих MR (`review-before-completion`), удаление worktree после merge. После успешного прогона можно предложить записать итог в daily note (`work-daily-log`) - одной строкой, без навязывания.
