...
- `squash`: в команде принято сквошить MR - всегда передавай `squash=true`. Политика проекта `encourage`/`always` действует на UI-форму и на merge, но MR, созданный push options без флага, рождается с `squash=false` - так в master уехали несквошенные коммиты MR !100 и !103.
...
`-o merge_request.squash` обязателен: без него MR создается со снятой галочкой squash независимо от политики проекта (см. оговорку про `squash` выше).
