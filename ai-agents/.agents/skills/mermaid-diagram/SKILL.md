...
- **OUTPUT_DIR** — Output directory for all generated files. Resolved at runtime: `$MERMAID_OUTPUT_DIR` if set, otherwise `figures/` relative to CWD.
...
```bash
# Resolve output directory: prefer MERMAID_OUTPUT_DIR env var, fall back to figures/
OUTPUT_DIR="${MERMAID_OUTPUT_DIR:-figures}"
mkdir -p "$OUTPUT_DIR"
...
if command -v mmdc &> /dev/null; then
    ...
    mmdc -i "$OUTPUT_DIR/<diagram-name>.mmd" -o "$OUTPUT_DIR/<diagram-name>.png" -b transparent
    echo "✅ Syntax valid — PNG rendered to $OUTPUT_DIR/<diagram-name>.png"
else
    # Try npx as fallback
    npx -y @mermaid-js/mermaid-cli@latest -i "$OUTPUT_DIR/<diagram-name>.mmd" -o "$OUTPUT_DIR/<diagram-name>.png" -b transparent
    echo "✅ Syntax valid — PNG rendered to $OUTPUT_DIR/<diagram-name>.png"
fi
```
...
### Files Generated
- `$OUTPUT_DIR/<diagram-name>.mmd` — Raw Mermaid source
- `$OUTPUT_DIR/<diagram-name>.md` — Markdown with embedded diagram
- `$OUTPUT_DIR/<diagram-name>.png` — Rendered PNG (if mmdc available)
...
Files:
  $OUTPUT_DIR/<diagram-name>.mmd  — Raw Mermaid source (use with mmdc, editors, CI)
  $OUTPUT_DIR/<diagram-name>.md   — Markdown preview (renders on GitHub, VS Code, etc.)
  $OUTPUT_DIR/<diagram-name>.png  — Rendered image (if mmdc was available)

To re-render manually:
  mmdc -i "$OUTPUT_DIR/<diagram-name>.mmd" -o "$OUTPUT_DIR/<diagram-name>.png"
```
...
```
$OUTPUT_DIR/   # $MERMAID_OUTPUT_DIR if set, else figures/ relative to CWD
...
1. **ALWAYS save files to `$OUTPUT_DIR`** (resolved in Step 0) — Never just output code in chat
