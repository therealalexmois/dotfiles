...
# worktree always lives at `<repo>/.worktrees/<dir>`, so `../../.claude` resolves
# back to the repo root. Best-effort: never aborts a created worktree.
if [ -e "$repo_root/.claude" ] && [ ! -e "$path/.claude" ]; then
  ln -s "../../.claude" "$path/.claude" 2>/dev/null || err "could not link .claude"
fi
